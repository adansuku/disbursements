...
RSpec.describe MonthlyFee, type: :model do
  ...
  describe 'validations' do
    it 'is valid with valid attributes' do
      monthly_fee = build(:monthly_fee, merchant:)
    ...
    it 'is not valid without month' do
      monthly_fee = build(:monthly_fee, merchant:, month: nil)
      expect(monthly_fee).not_to be_valid
    end

    it 'is not valid without amount' do
      monthly_fee = build(:monthly_fee, merchant:, amount: nil)
      expect(monthly_fee).not_to be_valid
    end
  end
  ...
  describe 'associations' do
    it 'belongs to a merchant' do
      monthly_fee = create(:monthly_fee, merchant:)
  ...
  describe 'monthly_disbursement_report', :focus do
    it 'returns the monthly disbursement report' do
      ...
      create(:monthly_fee, merchant:, month: Time.current.beginning_of_month.advance(years: -1))
      create(:monthly_fee, merchant:, month: 1.month.ago.beginning_of_month)
