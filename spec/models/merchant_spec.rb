...
RSpec.describe Merchant, type: :model do
  ...
  describe '#pending_daily_orders_for_disbursement' do
    it 'returns orders that are pending for daily disbursement' do
      today_order = create(:order, merchant:, created_at: Time.zone.now)
      yesterday_order = create(:order, merchant:, created_at: Time.zone.now - 1.day)
      order_with_disbursement = create(:order, merchant:, created_at: Time.zone.now - 1.day,
