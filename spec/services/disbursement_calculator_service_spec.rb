...
RSpec.describe DisbursementCalculatorService, type: :service do
  ...
  let(:date) { Date.today }
  let(:orders) { create_list(:order, 3, merchant:, created_at: date) }
  let(:service) { DisbursementCalculatorService.new(date, orders, merchant) }
  ...
  describe '#calculate_and_create_disbursement' do
    context 'when orders are present' do
      ...
      it 'rolls back the transaction on error' do
        ...
        disbusement_orders = create_list(:order, 3, merchant:, created_at: date, disbursement: disbusement)

        DisbursementCalculatorService.new(date, disbusement_orders, merchant).calculate_and_create_disbursement
